...
# Provides the `vclaude` command for managing devcontainers
...
Commands:
    ...
    self-install        Install 'vclaude' command to ~/.local/bin
    update              Update vclaude to the latest version
    ...
    claude [args]       Launch Claude Code inside the container (routed through gateway)
    admin               Show gateway stats and client list
    admin --expose      Forward gateway port to host (dashboard access)
    help                Show this help message

Examples:
    vclaude .                      # Install template and start container
    vclaude up                     # Start container in current directory
    vclaude claude                 # Launch Claude Code through the gateway
    vclaude admin                  # Show gateway stats
    vclaude admin --expose         # Access dashboard from host browser
    vclaude shell                  # Open interactive shell
    vclaude rebuild                # Clean rebuild
    vclaude self-install           # Install vclaude to PATH
    vclaude exec ls -la            # Run command in container
    vclaude mount ~/data /data     # Add mount to container
    vclaude sync                   # Sync sessions from all devcontainers
    vclaude sync crypto            # Sync only matching devcontainer
    vclaude cp /some/file ./out    # Copy a path from container to host
    vclaude destroy                # Remove all project Docker resources
    vclaude destroy -f             # Skip confirmation prompt
EOF
}

log_info() {
  echo -e "${BLUE}[vclaude]${NC} $1"
}

log_success() {
  echo -e "${GREEN}[vclaude]${NC} $1"
}

log_warn() {
  echo -e "${YELLOW}[vclaude]${NC} $1"
}

log_error() {
  echo -e "${RED}[vclaude]${NC} $1" >&2
}
...
cmd_up() {
  ...
  # Ensure .gateway-data/ exists on host (bind-mounted into container)
  mkdir -p "$workspace_folder/.gateway-data"

  log_info "Starting devcontainer in $workspace_folder..."

  devcontainer up --workspace-folder "$workspace_folder"
  log_success "Devcontainer started (gateway auto-starts inside)"
}
...
" &
    ...
    devcontainer exec --workspace-folder "$workspace_folder" \
      curl -sk https://localhost:8443/_health 2>/dev/null | python3 -m json.tool 2>/dev/null || echo "  Gateway not running"
    echo ""
    echo -e "${BLUE}=== Gateway Stats ===${NC}"
    local admin_token
    admin_token=$(devcontainer exec --workspace-folder "$workspace_folder" \
      cat /opt/.gateway-data/.admin-token 2>/dev/null || echo "")
    if [[ -n "$admin_token" ]]; then
      devcontainer exec --workspace-folder "$workspace_folder" \
        curl -sk -H "Authorization: Bearer $admin_token" https://localhost:8443/_admin/stats 2>/dev/null | python3 -m json.tool 2>/dev/null
      echo ""
      echo -e "${BLUE}=== Clients ===${NC}"
      devcontainer exec --workspace-folder "$workspace_folder" \
        curl -sk -H "Authorization: Bearer $admin_token" https://localhost:8443/_admin/clients 2>/dev/null | python3 -m json.tool 2>/dev/null
    else
...
cmd_mount() {
  ...
  if [[ -z "$host_path" ]] || [[ -z "$container_path" ]]; then
    log_error "Usage: vclaude mount <host_path> <container_path> [--readonly]"
    exit 1
  fi
  ...
  if [[ ! -f "$devcontainer_json" ]]; then
    log_error "No devcontainer.json found. Run 'vclaude template' first."
    exit 1
  fi
...
cmd_cp() {
  ...
  if [[ -z "$container_path" ]] || [[ -z "$host_path" ]]; then
    log_error "Usage: vclaude cp <container_path> <host_path>"
    exit 1
  fi
...
cmd_self_install() {
  ...
  local install_path="$install_dir/vclaude"
  ...
  log_success "Installed 'vclaude' to $install_path"
